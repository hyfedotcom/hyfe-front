...
export function ResourceCard({ card }: { card: ResourceCardType }) {
  ...
  return (
    <div
      className={
        "group flex h-auto md:h-full w-full flex-col border-[1.5px] border-border hover:border-primary hover:shadow-hover hover:bg-activ duration-300 space-y-4 md:space-y-5 bg-card rounded-[20px] overflow-hidden"
      }
    >
      <Image
        src={cover.url || ""}
